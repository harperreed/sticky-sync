...
fn test_categorize_new_on_filesystem() {
    ...
    let fs_times = HashMap::from([("uuid-1".to_string(), 2000), ("uuid-2".to_string(), 2000)]);

    let actions = SyncEngine::categorize(&fs_uuids, &db_times, &fs_times);

    assert!(actions
        .iter()
        .any(|a| matches!(a, SyncAction::NewOnFilesystem(uuid) if uuid == "uuid-1")));
}
...
fn test_categorize_modified_last_write_wins() {
    ...
    assert!(actions
        .iter()
        .any(|a| matches!(a, SyncAction::UpdateDatabase(uuid) if uuid == "uuid-1")));
}
...
fn test_categorize_db_newer() {
    ...
    assert!(actions
        .iter()
        .any(|a| matches!(a, SyncAction::UpdateFilesystem(uuid) if uuid == "uuid-1")));
}
...
fn test_categorize_no_change() {
    ...
    assert!(actions
        .iter()
        .any(|a| matches!(a, SyncAction::NoChange(uuid) if uuid == "uuid-1")));
}

#[test]
fn test_categorize_new_in_database() {
    let fs_uuids = vec!["uuid-1".to_string()];
    let db_times = HashMap::from([("uuid-1".to_string(), 2000), ("uuid-2".to_string(), 3000)]);
    let fs_times = HashMap::from([("uuid-1".to_string(), 2000)]);
    ...
    assert!(actions
        .iter()
        .any(|a| matches!(a, SyncAction::NewInDatabase(uuid) if uuid == "uuid-2")));
}
...
fn test_categorize_mixed_actions() {
    ...
    assert!(actions
        .iter()
        .any(|a| matches!(a, SyncAction::UpdateDatabase(uuid) if uuid == "uuid-2")));
    assert!(actions
        .iter()
        .any(|a| matches!(a, SyncAction::UpdateFilesystem(uuid) if uuid == "uuid-3")));
    assert!(actions
        .iter()
        .any(|a| matches!(a, SyncAction::NoChange(uuid) if uuid == "uuid-4")));
    assert!(actions
        .iter()
        .any(|a| matches!(a, SyncAction::NewInDatabase(uuid) if uuid == "uuid-5")));
